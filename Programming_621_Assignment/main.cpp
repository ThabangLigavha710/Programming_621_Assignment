...
public:
    ...
    vector<int> linearSearchByName(vector<Customer> &customers, string name) {
        vector<int> results;
        for (int i = 0; i < customers.size(); i++) {
            string fullName = string(customers[i].getFirstName()) + " " + string(customers[i].getSurname());
...
void calculateInterestForAllAccounts(vector<Customer> &customers) {
    ...
    for (auto &c : customers) {
        double interest = 0;
        string accType = c.getAccountType();
        
        if (accType == "Savings") {
            interest = c.getBalance() * 0.04;
            cout << c.getAccountNumber() << " (Savings): R" << interest << " interest\n";
        } else if (accType == "Fixed Deposit") {
            interest = c.getBalance() * 0.08;
            cout << c.getAccountNumber() << " (Fixed Deposit): R" << interest << " interest\n";
        } else if (accType == "Cheque") {
            interest = c.getBalance() * 0.02;
            cout << c.getAccountNumber() << " (Cheque): R" << interest << " interest\n";
        }
    }
}
...
void exportToCSV() {
    ...
    while (in.read((char*)&c, sizeof(c))) {
        out << c.getAccountNumber() << "," << c.getFirstName() << "," << c.getSurname() << ","
            << c.getIDNumber() << "," << c.getContact() << "," << c.getEmail() << ","
            << c.getAddress() << "," << c.getDOB() << "," << c.getAccountType() << ","
            << c.getBranch() << "," << c.getBalance() << "\n";
    }
...
void exportToText() {
    ...
    while (in.read((char*)&c, sizeof(c))) {
        ...
        out << "ID: " << c.getIDNumber() << "\n";
        out << "Contact: " << c.getContact() << "\n";
...
void customerAccountSummary() {
    ...
    while (file.read((char*)&c, sizeof(c))) {
        count++;
        total += c.getBalance();
        cout << count << ". " << c.getAccountNumber() << " | " 
             << c.getFirstName() << " " << c.getSurname() << " | R" << c.getBalance() << "\n";
    }
    cout << "----------------------------------------\n";
...
void addTeller() {
    ...
    cout << "Enter Teller ID (without T): ";
    cin >> id;
